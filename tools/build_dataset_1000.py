...
OUTPUT_DIR = f"{DATASET_ROOT}/images"
EXCEL_PATH = "/mnt/share_data/CT/ct_dataset_base_260316/0316_nodesk_ct_chest_1000.xlsx"
CSV_OUTPUT_PATH = f"{DATASET_ROOT}/reports.csv"
DCM2NIIX_PATH = os.environ.get("DCM2NIIX_PATH", "")
...
def is_thin_slice(json_path, nii_path):
    ...
    try:
        with open(json_path, 'r', encoding='utf-8') as f:
            ...
            t = parse_thickness_value(thickness)
            if t is not None:
                if 0 < t <= 3.0:
                    return True
                else:
...
def parse_thickness_value(thickness):
    ...
    if isinstance(thickness, str):
        thickness = thickness.strip().strip("[]")
        if "," in thickness:
            thickness = thickness.split(",")[0].strip()
        if "\\" in thickness:
            thickness = thickness.split("\\")[0].strip()
    try:
...
def main():
    ...
    dcm2niix_path = DCM2NIIX_PATH or shutil.which("dcm2niix")
    if not dcm2niix_path:
        raise RuntimeError(
            "dcm2niix not found. Set DCM2NIIX_PATH to the absolute binary path, or install/add dcm2niix to PATH."
        )
    ...
    csv_data = []
    convert_failed_cases = 0
    no_nii_cases = 0
    no_thin_slice_cases = 0
    failure_examples = []
    ...
    for folder in tqdm(target_subdirs, desc="DICOM 转换与图文匹配", unit="case"):
        ...
        try:
            ...
            if result.returncode != 0 and not os.listdir(patient_output_dir):
                convert_failed_cases += 1
                if len(failure_examples) < 5:
                    err_msg = (result.stderr or result.stdout or "").strip()
                    failure_examples.append(f"{folder_name}: {err_msg or 'dcm2niix exited with non-zero status'}")
                continue
        except Exception as e:
            convert_failed_cases += 1
            if len(failure_examples) < 5:
                failure_examples.append(f"{folder_name}: {type(e).__name__}: {e}")
            continue
    ...
    if not df_out.empty:
        ...
        print(f"convert_failed_cases={convert_failed_cases}, no_nii_cases={no_nii_cases}, no_thin_slice_cases={no_thin_slice_cases}")
        if failure_examples:
            print("Sample conversion failures:")
            for msg in failure_examples:
                print(f"  - {msg}")
