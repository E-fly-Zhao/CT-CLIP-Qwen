...
class CTCLIP(nn.Module):
    ...
    def forward(
        ...
        # 获取 Qwen 的最后层隐藏状态
        enc_text = text_outputs.last_hidden_state if hasattr(text_outputs, 'last_hidden_state') else text_outputs[0]

        # depending on whether text is using causal mask, post process, moving eos token to the first position

        # --- 2. 废弃原版 Causal Mask 逻辑 ---
        # 以下代码将其全部注释掉，Qwen 自带严格的因果掩码，强制修改会导致张量错乱：
       ...
       # print(enc_image.shape, flush=True)
        ...
        # 【替换为平均池化 Mean Pooling】：
        # 使用 attention_mask 屏蔽掉 padding 部分，仅对真实中文文本的特征求平均
        input_mask_expanded = text.attention_mask.unsqueeze(-1).expand(enc_text.size()).float()
        ...
        # ===================================================================
        # 🚨 跨卡全局对比学习核心逻辑 (已替换原版单卡矩阵计算)
        # ===================================================================
        
        # 确保在使用前引入了分布式模块
        import torch.distributed as dist

        # 取出主视图的特征 (去除多视角维度 m，此时 shape 为 [b, dim])
        local_text_latents = text_latents[0]
        local_image_latents = image_latents[0]

        # 兼容 CLOOB 的额外投影机制
        local_text_latents_extra = text_latents_extra[0] if self.extra_latent_projection else local_text_latents
        ...
        global_text_latents_extra = all_gather_with_grad(local_text_latents_extra)
