...
def main():
    ...
    with torch.no_grad():
        for pathology in tqdm(pathologies, desc="Caching Text Prompts"):
            ...
            # 使用 attention_mask 找到每句话真实的最后一个 Token 索引
            attention_mask = tokens['attention_mask']
            sequence_lengths = attention_mask.sum(dim=1) - 1  # 真实长度减1等于最后一个字的索引
            batch_size = outputs.last_hidden_state.shape[0]

            # 精准提取每句话的有效句式表征
            pooled_output = outputs.last_hidden_state[torch.arange(batch_size, device=device), sequence_lengths, :]

            # 穿过文本投影层并进行 L2 归一化
            text_latent = clip.to_text_latent(pooled_output)
    ...
    # 2. 地毯式扫荡 clip 内部的所有文本模型别名
    if hasattr(clip, 'text_encoder'):
        del clip.text_encoder
    if hasattr(clip, 'text_transformer'):
        del clip.text_transformer
    ...
    with torch.no_grad():
        for img_tensor, labels, patient_id in tqdm(dataloader, desc="Inference"):
            # 1. 强转为安全的 Float32，屏蔽 VQ 精度 Bug
            img_tensor = img_tensor.to(device, dtype=torch.float32).contiguous()

            # ==========================================================
            # 🚨 完美拦截法 🚨
            try:
                # 正常送入模型，它会自动执行：完美切块 -> 编码 -> 量化
                clip.visual_transformer(img_tensor)
            except TokenEscape as e:
                # 触发警报！在到达 mse_loss 之前瞬间中止，并成功接住纯净特征！
                quantized_tokens = e.tokens

            # 👇 🚨 终极密码破解：沿深度方向 (Depth=1) 进行均值池化！
            # [1, 24, 24, 24, 512] ---> [1, 24, 24, 512]
            pooled_tokens = quantized_tokens.mean(dim=1)

            # 强行把特征压平：24 * 24 * 512 = 294912，完美契合！
            flattened_tokens = pooled_tokens.view(pooled_tokens.size(0), -1)
            
            # 穿过满血版投影层，来到 512 维的对比空间
            image_latent = clip.to_visual_latent(flattened_tokens)
            # ==========================================================
            ...
            # 🎯 降维打击：并行矩阵乘法
            # [1, 512] @ [512, num_pathologies * 2] -> [1, num_pathologies * 2]
    ...
    all_predictions = np.array(all_predictions) # 模型输出的概率矩阵 Shape: [Batch, 431]
    all_labels = np.array(all_labels)           # 真实标签矩阵 Shape: [Batch, 431]

    # 将概率通过 0.5 阈值二值化，用于计算 F1, ACC, Pre, Rec
    ...
    for i, pathology in enumerate(pathologies):
        ...
        # 只有当该疾病在测试集中既有正样本又有负样本时，指标计算才有数学意义
        if len(np.unique(y_true)) > 1:
            auc = roc_auc_score(y_true, y_prob)
            # zero_division=0 防止预测全为0或全为1时报除零警告
            ...
            # 遇到全 0 或全 1 的极端情况，记为 N/A，跳过不计入平均值
    ...
    for key in valid_metrics:
        if len(valid_metrics[key]) > 0:
            avg_metrics[key] = np.mean(valid_metrics[key])
        else:
            avg_metrics[key] = "N/A"
