...
def main():
    ...
    results_folder = "./qwen_zeroshot_2000_threshold/"
    ...
    # ==========================================================
    # A. 按疾病计算：引入 Youden's J Index 动态自适应阈值机制
    # ==========================================================
    disease_results_list = []
    valid_disease_metrics = {'AUROC': [], 'F1': [], 'Accuracy': [], 'Precision': [], 'Recall': []}
    
    # 🚨 新增：创建每种疾病的专属最佳阈值存钱罐，供下方样本级评估使用
    best_thresholds_per_disease = np.zeros(len(pathologies), dtype=np.float32)

    print("🎯 正在启动约登指数(Youden's J)自适应阈值搜索...")
    for i, pathology in enumerate(pathologies):
        ...
        if len(np.unique(y_true)) > 1:
            ...
            # 约登指数 J = TPR - FPR，求最大值对应的阈值
            idx_best = np.argmax(tpr - fpr)
            best_thresh = thresholds[idx_best]
            
            # 极致防极端截断
            if best_thresh > 0.99 or best_thresh < 0.01:
                best_thresh = 0.5
                
            best_thresholds_per_disease[i] = best_thresh

            # 3. 用专属黄金阈值进行二值化判决
            y_pred_dynamic = (y_prob >= best_thresh).astype(np.int32)

            f1 = f1_score(y_true, y_pred_dynamic, zero_division=0)
            acc = accuracy_score(y_true, y_pred_dynamic)
            pre = precision_score(y_true, y_pred_dynamic, zero_division=0)
            ...
            metrics.update({
                ...
                "Precision": pre, 
                "Recall": rec,
                "Best_Threshold": best_thresh # 记录在 Excel 里，方便查阅
            })
            ...
            best_thresholds_per_disease[i] = 0.5
            metrics.update({"AUROC": "N/A", "F1": "N/A", "Accuracy": "N/A", "Precision": "N/A", "Recall": "N/A", "Best_Threshold": 0.5})

        disease_results_list.append(metrics)

    disease_avg_metrics = {"Pathology": "Disease Macro Average (Valid Only)", "Best_Threshold": "-"}
    ...
    df_disease_results.to_excel(os.path.join(results_folder, "qwen_zeroshot_disease_macro_metrics.xlsx"), index=False)

    # ==========================================================
    # B. 按患者计算：同步应用每种疾病的专属最佳阈值
    # ==========================================================
    ...
    for idx in range(all_labels.shape[0]):
        ...
        y_prob = all_predictions[idx, :]
        
        # 🚨 修正：每个疾病位使用自己专属的最佳阈值
        y_pred = (y_prob >= best_thresholds_per_disease).astype(np.int32)
